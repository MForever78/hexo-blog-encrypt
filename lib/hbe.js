...
const knownPrefix = "<hbe-prefix></hbe-prefix>";
  ...
  async function convertHTMLToElementAndAppend(content, parent) {
    parent.innerHTML = content;
    parent.querySelectorAll('script').forEach(async (elem) => {
  ...
  async function decrypt(decryptKey, iv, hmacKey) {
    ...
    const result = await cryptoObj.subtle.decrypt({
      ...
      // document.getElementById('hexo-blog-encrypt').parentElement.replaceChild(await convertHTMLToElement(decoded), mainElement);
	  await convertHTMLToElementAndAppend(decoded, mainElement.parentElement);
